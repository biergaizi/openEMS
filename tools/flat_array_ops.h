...
struct Flat_3DArray
{
	...
	const inline T operator() (const unsigned int &x, const unsigned int &y, const unsigned int &z) const
	{
		return array[
		           x * x_stride +
		           y * y_stride +
...
template <typename T>
Flat_3DArray<T>* Create_Flat_3DArray(const unsigned int* numLines)
{
	unsigned int x_max = numLines[0];
	unsigned int y_max = numLines[1];
	unsigned int z_max = numLines[2];

	// size of the Flat_3DArray class itself.
	size_t size = sizeof(Flat_3DArray<T>);

	// and the actual memory of the array[0] flexible array member
	size += sizeof(T) * x_max * y_max * z_max;
	...
	// create Flat_3DArray inside manually allocated memory "buf".
	Flat_3DArray<T>* array = new (buf) Flat_3DArray<T>;
...
template <>
inline Flat_3DArray<f4vector>* Create_Flat_3DArray<f4vector>(const unsigned int* numLines)
{
	unsigned int x_max = numLines[0];
	unsigned int y_max = numLines[1];
	unsigned int z_max = ceil((double) numLines[2] / 4.0);

	// If the definition of Flat_3DArray<f4vector> has been changed,
	...
	static_assert(offsetof(Flat_3DArray<f4vector>, array) % F4VECTOR_SIZE == 0);

	// Size of the header itself.
	size_t size = sizeof(Flat_3DArray<f4vector>);

	// and the actual memory of the array[0] flexible array member
	size += F4VECTOR_SIZE * x_max * y_max * z_max;
	...
	// create Flat_3DArray inside manually allocated memory "buf".
	Flat_3DArray<f4vector>* array = new (buf) Flat_3DArray<f4vector>;
	array->x_stride = y_max * z_max;
	array->y_stride = z_max;
...
template <typename T>
void Delete_Flat_3DArray(Flat_3DArray<T>* array, const unsigned int* numLines)
{
	if (!array)
	{
		return;
