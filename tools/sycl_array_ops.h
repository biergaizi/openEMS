...
template <>
inline ArrayXYZN<sycl::float4>* CreateArrayXYZN<sycl::float4>(sycl::queue Q, const unsigned int* numLines)
{
        unsigned int n_max = 3;
        unsigned int x_max = numLines[0];
        unsigned int y_max = numLines[1];
        ...
        sycl::float4* buf = sycl::malloc_device<sycl::float4>(size, Q);
        //memset(buf, 0, size * sizeof(T));

        // allocate wrapper class
        ArrayXYZN<sycl::float4>* array = new ArrayXYZN<sycl::float4>();
        array->n_stride = n_stride;
        array->x_stride = x_stride;
        array->y_stride = y_stride;
        array->array = buf;
