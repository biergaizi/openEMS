...
void Engine_Ext_Mur_ABC::DoPreVoltageUpdatesImpl(Engine* eng, int threadID)
{
	...
	for (unsigned int lineX=0; lineX<m_numX.at(threadID); ++lineX)
	{
		...
		for (pos[m_nyPP]=0; pos[m_nyPP]<m_numLines[1]; ++pos[m_nyPP])
		{
			...
			m_volt_nyP[pos[m_nyP]][pos[m_nyPP]] = eng->Engine::GetVolt(m_nyP,pos_shift) - m_Op_mur->m_Mur_Coeff_nyP[pos[m_nyP]][pos[m_nyPP]] * eng->Engine::GetVolt(m_nyP,pos);
			m_volt_nyPP[pos[m_nyP]][pos[m_nyPP]] = eng->Engine::GetVolt(m_nyPP,pos_shift) - m_Op_mur->m_Mur_Coeff_nyPP[pos[m_nyP]][pos[m_nyPP]] * eng->Engine::GetVolt(m_nyPP,pos);
		}
	}
}
...
template <typename Engine>
void Engine_Ext_Mur_ABC::DoPostVoltageUpdatesImpl(Engine* eng, int threadID)
{
	if (IsActive()==false) return;
	if (m_Eng==NULL) return;
	if (threadID>=m_NrThreads)
	...
	for (unsigned int lineX=0; lineX<m_numX.at(threadID); ++lineX)
	{
		...
		{
			pos_shift[m_nyPP] = pos[m_nyPP];
			m_volt_nyP[pos[m_nyP]][pos[m_nyPP]] += m_Op_mur->m_Mur_Coeff_nyP[pos[m_nyP]][pos[m_nyPP]] * eng->Engine::GetVolt(m_nyP,pos_shift);
			m_volt_nyPP[pos[m_nyP]][pos[m_nyPP]] += m_Op_mur->m_Mur_Coeff_nyPP[pos[m_nyP]][pos[m_nyPP]] * eng->Engine::GetVolt(m_nyPP,pos_shift);
		}
	}
}
...
template <typename Engine>
void Engine_Ext_Mur_ABC::Apply2VoltagesImpl(Engine* eng, int threadID)
{
	if (IsActive()==false) return;
	if (threadID>=m_NrThreads)
		return;
	...
	for (unsigned int lineX=0; lineX<m_numX.at(threadID); ++lineX)
	{
		...
		for (pos[m_nyPP]=0; pos[m_nyPP]<m_numLines[1]; ++pos[m_nyPP])
		{
			eng->Engine::SetVolt(m_nyP,pos, m_volt_nyP[pos[m_nyP]][pos[m_nyPP]]);
			eng->Engine::SetVolt(m_nyPP,pos, m_volt_nyPP[pos[m_nyP]][pos[m_nyPP]]);
		}
	}
}
...
{
	ENG_THREAD_DISPATCH(Apply2VoltagesImpl, threadID);
}
