...
void Engine_Ext_Dispersive::Apply2VoltagesImpl(Engine* eng)
{
	for (int o=0;o<m_Op_Ext_Disp->m_Order;++o)
	{
		...
		{
			eng->Engine::SetVolt(0,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetVolt(0,pos[0][i],pos[1][i],pos[2][i]) - volt_ADE[o][0][i]
			);
			eng->Engine::SetVolt(1,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetVolt(1,pos[0][i],pos[1][i],pos[2][i]) - volt_ADE[o][1][i]
			);
			eng->Engine::SetVolt(2,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetVolt(2,pos[0][i],pos[1][i],pos[2][i]) - volt_ADE[o][2][i]
			);
		}
	}
}
...
template <typename Engine>
void Engine_Ext_Dispersive::Apply2CurrentImpl(Engine* eng)
{
	for (int o=0;o<m_Op_Ext_Disp->m_Order;++o)
	{
		...
		for (unsigned int i=0; i<m_Op_Ext_Disp->m_LM_Count.at(o); ++i)
		{
			eng->Engine::SetCurr(0,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetCurr(0,pos[0][i],pos[1][i],pos[2][i]) - curr_ADE[o][0][i]
			);
			eng->Engine::SetCurr(1,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetCurr(1,pos[0][i],pos[1][i],pos[2][i]) - curr_ADE[o][1][i]
			);
			eng->Engine::SetCurr(2,pos[0][i],pos[1][i],pos[2][i],
				eng->Engine::GetCurr(2,pos[0][i],pos[1][i],pos[2][i]) - curr_ADE[o][2][i]
			);
		}
	}
}
