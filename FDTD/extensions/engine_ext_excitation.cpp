...
void Engine_Ext_Excitation::Apply2VoltagesImpl(Engine *eng)
{
	...
	for (unsigned int n=0; n<m_Op_Exc->Volt_Count; ++n)
	{
		...
		eng->Engine::SetVolt(ny,pos, eng->Engine::GetVolt(ny,pos) + m_Op_Exc->Volt_amp[n]*exc_volt[exc_pos]);
	}
}
...
template <typename Engine>
void Engine_Ext_Excitation::Apply2CurrentImpl(Engine *eng)
{
	...
	for (unsigned int n=0; n<m_Op_Exc->Curr_Count; ++n)
	{
		...
		eng->Engine::SetCurr(ny,pos, eng->Engine::GetCurr(ny,pos) + m_Op_Exc->Curr_amp[n]*exc_curr[exc_pos]);
	}
}
